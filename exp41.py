...
def train(trainloader, model, nll, optimizer, epoch, use_cuda, lam):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        c       = angleW.mm(angleWt) # 10 x 10
        c       = c.view(-1, 1)

        dc      = c.data
        _, indices = dc.topk(global_num_classes + 1, 0)
        ind     = torch.LongTensor([indices[global_num_classes][0]])
        worst = int(indices[global_num_classes][0])
        wa    = worst / global_num_classes
        wb    = worst % global_num_classes
        print('worst_two: %d vs %d ' % (wa, wb), file = global_writer)
        ind     = Variable(ind, requires_grad = False).cuda()
        cos     = c.index_select(0, ind) # torch.LongTensor([indices[10][0]]))
        #W       = angleWt.mm(fixed)
        #Wt      = W.t()
        #cos     = Wt.mm(W)
        cos     = cos * lam
