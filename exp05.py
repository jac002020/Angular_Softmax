...
def train(trainloader, model, nll, optimizer, epoch, use_cuda, lam):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        dc      = c.data
        _, indices = dc.topk(11, 0)
        ind     = torch.LongTensor([indices[10][0]])
        #print('indices = ', indices)
        #print('indices[10][0] = ', indices[10][0])
        #print(ind)
        worst = int(indices[10][0])
        wa    = worst / 10
        wb    = worst % 10
        print('worst_two: %d vs %d (%s vs %s)' % (wa, wb, classes[wa], classes[wb]), file = global_writer)
