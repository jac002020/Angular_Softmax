...
def main():
    ...
    # Define a-softmax learning, choose to optimize only body, hand optimize angleW
    nll       = nn.CrossEntropyLoss()
    #optimizer = optim.SGD(model.parameters(), lr = args.lr, momentum = args.momentum, weight_decay = args.weight_decay)
    core      = [x for x in model.parameters()]
    cnt_fc    = 0
    for m in model.modules():
        if isinstance(m, nn.Linear) and 'out_features' in m.__dict__ and m.out_features == num_classes:
            globalM = m
            if globalM.bias:
                core = core[:-2]
            else:
            ...
            cnt_fc  += 1
    assert(cnt_fc == 1)
    optimizer = optim.SGD(core, lr = args.lr, momentum = args.momentum, weight_decay = args.weight_decay)
...
def train(trainloader, model, nll, optimizer, epoch, use_cuda, lam):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        loss.backward()
        optimizer.step()
        # update globalM's weight, must assure the norm == 1 conditions
        assert(globalM.weight.data[0][0] == angleW.data[0][0])
        # use globalM.weight.grad.data
        fenzi = globalM.weight.data * globalM.weight.grad.data
        fenzi = fenzi.sum(1) * 2
        #print('fenzi size = ', fenzi.size())
        fenmu = globalM.weight.grad.data.norm(2, 1)
        fenmu = fenmu * fenmu
        alpha = fenzi / fenmu
        #alpha = F.relu(alpha)
        alpha[alpha < 0] = 0
        nz    = alpha.view(-1).nonzero()
        print('alpha nonzero number = ', nz.size(0), file = global_writer)
        print('alpha mean = ', alpha.mean(), file = global_writer)
        print('alpha max = ',  alpha.max(), file = global_writer)
        globalM.weight.data -= globalM.weight.grad.data * alpha.expand_as(globalM.weight.grad.data)
        ...
        # plot progress
        bar.suffix = '({batch} / {size}) | Batch: {bt:.3f}s | Total: {total:} | Loss: {loss:.4f} | top1: {top1: .4f} | top5: {top5: .4f} | mean_W: {mw: .4f} | Old_Loss: {old_loss: .4f} | Cos_Loss: {cos_loss:.4f}'.format(
            batch = batch_idx + 1,
